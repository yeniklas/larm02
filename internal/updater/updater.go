...
const releaseAPI = "https://api.github.com/repos/yeniklas/larm02/releases/latest"
...
func Run(currentVersion string) error {
	...
	if rel.TagName == currentVersion {
		fmt.Printf("larm02 is already up to date (%s)\n", currentVersion)
		return nil
	}

	assetName := fmt.Sprintf("larm02-%s-%s-%s", rel.TagName, runtime.GOOS, runtime.GOARCH)
	...
	probe, err := os.CreateTemp(dir, ".larm02-update-*")
	if err != nil {
	...
	fmt.Printf("Update larm02 %s → %s? [y/N] ", currentVersion, rel.TagName)
	...
	tmp, err := os.CreateTemp(dir, ".larm02-update-*")
	if err != nil {
